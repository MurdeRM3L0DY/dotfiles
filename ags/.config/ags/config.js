...
const Clock = () => {
  const UPDATE_MILLIS = 60000;
  const now = () => GLib.DateTime.new_now_local();
  const timeout = () => UPDATE_MILLIS - (now().get_seconds() * 1000) % UPDATE_MILLIS;
  const format = () => now().format('%H:%M %a %d/%m') || '';

  return Widget.Label({
    class_name: 'clock',
    setup: self => {
      const initTimeout = Variable(null, {
        poll: [timeout(), (_) => null]
      });

      self.hook(initTimeout, self => {
        self.label = format();
      });

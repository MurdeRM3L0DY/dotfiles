-- yanked from packer.nvim
-- trying to wrap my head around lua coroutines
...
local step = function(func, callback)
  local thread = thread_create(func)
  local tick = nil
  tick = function(...)
    local ok, fn_or_err = resume(thread, ...)
    if ok then
      if type(fn_or_err) == 'function' then
        fn_or_err(tick)
      else
        (callback or noop)(fn_or_err)
      end
    else
      (callback or noop)(nil)
    end
  end
